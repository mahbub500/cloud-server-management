...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "csm_{$_args['post_type']}", 'csm' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "csm_{$_args['post_type']}", $posts, 'csm', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'csm' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'csm_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /cloud-server-management directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with cloud-server-management_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'csm_template_overwrite_dir', get_stylesheet_directory() . '/cloud-server-management/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( CLOUD_SERVER_MANAGEMENT ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'cloud-server-management' );
		}
	}
}